...
**Never name a custom Qt `Property` after a built-in `QWidget` property** (`pos`, `size`, `geometry`, `x`, `y`, `width`, `height`, `enabled`, …). A `Property(float, …)` named `pos` on a `QWidget` subclass *shadows* `QWidget.pos` (the geometry position), so `QPropertyAnimation(self, b"pos")` animates the **widget's position** to your float values — flinging the widget to `(0,0)`. The `ToggleSwitch` knob animation hit this exactly (toggle jumped to the row's top-left on click); the property was renamed `pos`→`knob`. Use a distinct name + a headless `qapp` regression test asserting `widget.x()` doesn't move (see `tests/test_toggle_switch.py`).
...
**`PinnedModelCard` bands share ONE left rail with the provider rows.** Any header band (Arena crest, Speed velocity, future bands) places its emblem at `_icon_col_cx()` (the trust-seal column center) and its text/content at `_content_col_x()` (the provider-name column) — and the provider rows use the same two helpers. Bands stack with a uniform vertical rhythm (`BAND_GAP` between bands, `ROWS_GAP` before the rows; pills fill their `CREST_H`/`SPEED_H` slot with no asymmetric inset). Don't hardcode per-band x/gap offsets — drift from the rail is exactly the misalignment we hunted down. Locked by `tests/test_speed_percentile.py::test_crest_and_speed_bands_share_columns`.
...
- **HTTP 401 / $0.00 means an *empty* API key, not an invalid one.** `config._load_api_key()` resolves `OPENROUTER_API_KEY` (env) FIRST, then `settings.json` `api_key`. A blank result sends `Authorization: Bearer ` → 401. Diagnose from the *launch* shell: `python -c "import config; print(len(config.API_KEY))"` (0 = no key). `pulse-rebuild` does `Start-Process`, so the exe inherits that shell's env — a stale/empty session env var wins over a valid `settings.json`. (A persistent User-scope `OPENROUTER_API_KEY` is set on the dev machine so fresh shells are authed.) **Caution:** assistant tool file/log reads can be an isolated FS snapshot that disagrees with the user's live machine — confirm 401-class symptoms from the user's own shell, not tool reads.
