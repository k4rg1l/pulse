...
A Windows tray app that watches your OpenRouter balance, burn rate, and auto top-ups at a glance. Dark themed, frameless, stays out of your way.
...
```powershell
git clone https://github.com/k4rg1l/openrouter-pulse.git
cd openrouter-pulse
...
If you'd rather not use the env var, run the app once to generate `%APPDATA%\OpenRouterPulse\settings.json`, then put your key in the `api_key` field there. The tray menu has an "Open Settings File..." entry that opens it in your default editor.
...
All settings live in `%APPDATA%\OpenRouterPulse\settings.json`:
